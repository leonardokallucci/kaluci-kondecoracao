...
import { getPendingWithdrawalsAdmin, approveWithdrawal, denyWithdrawal } from '@/lib/api'
import { genCoupon } from '@/lib/util'
...
export default function AprovacoesAdmin() {
  ...
  return (
    <div className="space-y-4">
      {msg && <div className="p-3 rounded-lg" style={{background:'#E8F5E9', border:'1px solid #C8E6C9'}}>{msg}</div>}
      <div className="card">
              ...
              {items.map((w:any) => (
                <tr key={w.id} className="border-b">
                  ...
                  <td className="space-x-2">
                    <button onClick={() => onApprove(w.id, w.amount_koins)} className="px-3 py-1 rounded-lg" style={{background:'#2E7D32', color:'#fff'}}>Aprovar & gerar cupom</button>
                    <button onClick={() => onDeny(w.id)} className="px-3 py-1 rounded-lg" style={{background:'#C62828', color:'#fff'}}>Negar</button>
                  </td>
                </tr>
              ))}
            </tbody>
